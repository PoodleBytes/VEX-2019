...
void autonomous(void) {
  ...
  /*   closeClaw(0.85);  //grab preload
    ...
    rLift(-90,20,1);
    openClaw();
    rDrive(-250, -250, 50, 50, 1);   */

  //DT.driveFor(fwd, 12, vex::distanceUnits::in, 50, vex::velocityUnits::pct,true);
  DT.turnFor(vex::turnType::right, 90, vex::rotationUnits::deg, 30,
...
void aLift(double deg, double s, bool b) { // position lift by absolute position
  Lift.rotateTo(deg, vex::rotationUnits::deg, s, vex::velocityUnits::pct,
                b); // This command must be non blocking.
...
int updateScreen() {
  while (1) {
    ...
    Controller1.Screen.print("C A:%2.2f t:%2.2f", Claw.current(amp),
                             Claw.torque(vex::torqueUnits::InLb)); // LEFT MOTOR
    Controller1.Screen.setCursor(2, 1);
    Controller1.Screen.print(
        "L A:%2.2f t:%2.2f", Lift.current(amp),
        Lift.torque(vex::torqueUnits::InLb)); // RIGHT MOTOR
    Controller1.Screen.setCursor(3, 1);
    Controller1.Screen.print(
        "T C:%3.1f L:%3.1f",
        Claw.temperature(vex::temperatureUnits::fahrenheit),
        Lift.temperature(vex::temperatureUnits::fahrenheit)); // LIFT AND STAND

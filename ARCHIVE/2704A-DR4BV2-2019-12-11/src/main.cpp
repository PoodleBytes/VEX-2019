...
int dist2Grab = 150;    //mm to grab cube
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    //  mDrive(-15);
        drive2Target(dist2Grab);  //test drive2Target()
    }else if (Controller1.ButtonRight.pressing()) { // move right
      mDrive(5);
    }
    
/*     if(dist_mm < dist2Grab + 10 && dist_mm > dist2Grab - 10){   //distance to target within 20mm
      Controller1.rumble(".        ");            //short rumble
      //vex::task::sleep(150);
    }else if(dist_mm < dist2Grab - 10){     //too close
...
void drive2Target(double D) { // drive by spin
  ...
  double numDegToDrive;                             //track # of degrees motors turned enrout to target
  int speed;

  //rDrive(num_deg, num_deg, speed, speed,1);
  bool b = 1;
  while(b){

      if(dist_mm >99999){b = 0;}  //no object detected - quit

      while(dist_mm > D + 100){    //distance from object is greater than target distance
        numDegToDrive = numDegToTarget - L_Drive.rotation(deg);

        if(numDegToDrive/numDegToTarget > 0.4){
          speed = 60;
        } else {
          speed = numDegToDrive/numDegToTarget * 100 + 20;}
...
void mDrive(int i) { // middle drive by spin
  int speed = 20;
  M_Drive.spin(vex::directionType::fwd, speed * i, vex::velocityUnits::pct);
...
void setUpMotor(motor(M), char B) {
  M.setMaxTorque(100, percentUnits::pct); // set torque

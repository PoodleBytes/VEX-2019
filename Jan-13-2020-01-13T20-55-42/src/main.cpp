...
void autonomous(void) {
  ...
  closeClaw(50);          //grab preload cube - 1 CUBE
  
rLift(70,50,1); 

drive2Target(dist2Cube);
  //grabCube(1);         //EXP - GRAB NEXT - SINGLE  CUBE - 2 cubes
...
int tLift(void) { // ARM & CLAW TASK
  while (1) {
    if (Controller1.Axis3.value() > deadBand && Lift.position(vex::rotationUnits::deg) < 470) { // RAISE LIFT W/SOFT LIMIT OF 500 DEG
      ...
      Lift.spin(vex::directionType::fwd, Controller1.Axis3.value() * (adjLift * 0.3), vex::velocityUnits::pct);
...
void closeClaw(double speed) {
  ...
  } while (Claw.current(vex::percentUnits::pct) < 75);
  Claw.stop(hold);
} // end rClaw
...
void grabCube(int n){
  ...
  rLift(-25 ,20,1);     //tuoch top of cube 
  openClaw(Claw.rotation(deg)-20);  //open claw current position - 20 deg
  rLift(liftTo *-1, 20, 1);  //lower lift to start (home) position
...
int updateScreen() {
  while (1) {
    ...
    Controller1.Screen.print("L: %4f R: %4f", L_Drive.rotation(rotationUnits::deg), R_Drive.rotation(rotationUnits::deg)); // LEFT, RIGHT MOTOR
    Controller1.Screen.setCursor(2, 1);
    Controller1.Screen.print("L: %4f C: %4f", Lift.rotation(rotationUnits::deg), Claw.rotation(rotationUnits::deg)); // LIFT CLAW
    Controller1.Screen.setCursor(3, 1);
    Controller1.Screen.print("Distance: %5f", dist_mm);
